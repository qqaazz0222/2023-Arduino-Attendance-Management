...
const getWeek = async (gap) => {
    ...
    dstartInit.setDate(
        dstartInit.getDate() - dstartInit.getDay() + gap * 7 + 1
    );
    dendInit.setDate(dendInit.getDate() - dendInit.getDay() + gap * 7 + 5);
    dstart =
        dstartInit.getFullYear() +
        "-" +
        ("0" + (dstartInit.getMonth() + 1)).slice(-2) +
        "-" +
        ("0" + dstartInit.getDate()).slice(-2);
    dend =
        dendInit.getFullYear() +
        "-" +
        ("0" + (dendInit.getMonth() + 1)).slice(-2) +
        "-" +
        ("0" + dendInit.getDate()).slice(-2);
    ...
    const get_checkIn = await pool.query(
        "SELECT *, WEEKDAY(date) AS day FROM check_in WHERE DATE_FORMAT(date, '%Y-%m-%d') BETWEEN ? AND ? ORDER BY uid ASC;",
        [dstart, dend]
    );
    ...
    get_checkOut[0].forEach((e) => {
        ...
        var diff = Math.abs(
            parseInt((chkout.getTime() - chkin.getTime()) / (1000 * 60 * 60))
        );
        if (diff >= 4) {
        ...
        if (req.session.isAdmin != true) {
            ...
            try {
                ...
                const getAllMember = await pool.query(
                    "SELECT a.uid, a.uname, a.upw, a.seat, DATE_FORMAT(b.date, '%H시%i분%s초') AS 'in', DATE_FORMAT(c.date, '%H시%i분%s초') AS 'out', TIMESTAMPDIFF(HOUR, b.date, c.date) AS hour, abs(TIMESTAMPDIFF(HOUR, b.date, CURRENT_TIMESTAMP)) AS curhour FROM user AS a LEFT JOIN check_in AS b ON a.uid = b.uid AND DATE_FORMAT(b.date, '%Y-%m-%d') = ? LEFT JOIN check_out AS c ON a.uid = c.uid AND DATE_FORMAT(c.date, '%Y-%m-%d') = ? ORDER BY a.seat ASC;",
                    [key, key]
                );
        ...
        if (req.session.isAdmin != true) {
            ...
            try {
                ...
                const editUser = await pool.query(
                    "UPDATE user SET upw = ?, uname = ?, seat = ? WHERE uid = ?",
                    [userupw, username, userseat, studentcode]
                );
                return res.send(
    ...
    if (req.session.isLogined == undefined) {
        ...
        if (req.session.isAdmin != true) {
            ...
            try {
                ...
                return res.send(
                    "<script>alert('현재 엑셀 다운로드 기능이 준비 중입니다.'); location.href='/admin/manage';</script>"
                );
            } catch (error) {
                console.log(error);
                res.redirect("/error");

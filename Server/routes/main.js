...
router.get("/", async (req, res, next) => {
  ...
  if (req.session.isLogined == undefined) {
    ...
    try {
      ...
      const check_in = await pool.query("SELECT * FROM check_in WHERE DATE_FORMAT(date, '%Y-%m') BETWEEN ? AND ? AND uid = ?;", [
        String(year) + "-" + m1,
        yy + "-" + m2,
        req.session.uid,
      ]);
      // DB에서 기간에 해당하는 퇴실 정보 가져오기
      const check_out = await pool.query("SELECT * FROM check_out WHERE DATE_FORMAT(date, '%Y-%m') BETWEEN ? AND ? AND uid = ?;", [
        String(year) + "-" + m1,
        yy + "-" + m2,
        req.session.uid,
      ]);
      // DB에서 메인 페이지 좌측 하단에 표시할 공지 정보 가져오기
      const notice = await pool.query("SELECT * FROM notice ORDER BY no DESC LIMIT 4;");
...
router.post("/editpwd", async (req, res, next) => {
  ...
  if (req.session.isLogined == undefined) {
    ...
    try {
      ...
      const chk_last_pwd = await pool.query("SELECT * FROM user WHERE uid = ? AND upw = ?", [req.session.uid, current_user_pwd]);
      ...
      if (chk_last_pwd[0].length === 0) {
        ...
        const update_new_pwd = await pool.query("UPDATE user SET upw = ? WHERE uid = ?", [new_user_pwd.toLowerCase(), req.session.uid]);
      }
